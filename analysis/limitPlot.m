...
function limitPlot(plotStruct, Info, TransformerData, busIsLoad)
    ...
    for iProd = 1:nIter
        ...
        pvProd(iProd) = max(real(plotStruct(iProd).PvPowerPerLoad)).*TransformerData.S_base/1000;
    ...
    if strcmp(structName, 'SelectDist')
        ...
        plot(numSystems, zeros(length(numSystems),1), 'r-')
