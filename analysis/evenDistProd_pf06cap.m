...
P_pv=createComplexPower(P_pv,'P',0.6,1);
pvFactor = 1;
...
%for iPv = 0:length(loadNumber)      % iPV - number of pv systems
    ...
    EvenDist_pf06cap(iPv+1).Results.U_hist = res.U_hist(busIsLoad,:);          % Kanske ska ta bort denna då det ÄTER! RAM-minne
    EvenDist_pf06cap(iPv+1).Results.S_hist = res.S_hist(busIsLoad,:);
    EvenDist_pf06cap(iPv+1).PvSystemsAdded = iPv;
    EvenDist_pf06cap(iPv+1).PvPowerPerLoad = pvPerLoad;
    
    %Store interesting points in EvenDist.Critical
    EvenDist_pf06cap(iPv+1).Critical.maxVoltage.Voltage = abs(voltageVec(rowMaxLoad, timeMaxLoad));
    EvenDist_pf06cap(iPv+1).Critical.maxVoltage.TimeStamp = timeMaxLoad;
    EvenDist_pf06cap(iPv+1).Critical.maxVoltage.BusNumber = iMaxLoad;
    
    EvenDist_pf06cap(iPv+1).Critical.minVoltage.Voltage = abs(voltageVec(rowMinLoad, timeMinLoad));
    EvenDist_pf06cap(iPv+1).Critical.minVoltage.TimeStamp = timeMinLoad;
    EvenDist_pf06cap(iPv+1).Critical.minVoltage.BusNumber = iMinLoad;
    
    [maxMean, rowMaxMean] = max(mean(abs(voltageVec), 2));
    EvenDist_pf06cap(iPv+1).Critical.maxMeanVoltage.Voltage = maxMean;
    EvenDist_pf06cap(iPv+1).Critical.maxMeanVoltage.BusNumer = loadNumber(rowMaxMean);
    
    [minMean, rowMinMean] = min(mean(abs(voltageVec), 2));
    EvenDist_pf06cap(iPv+1).Critical.minMeanVoltage.Voltage = minMean;
    EvenDist_pf06cap(iPv+1).Critical.minMeanVoltage.BusNumber = loadNumber(rowMinMean);
    
    
    if iPv == 0
        EvenDist_pf06cap(iPv+1).Critical.deltaV.Voltage = 0;
        EvenDist_pf06cap(iPv+1).Critical.deltaV.TimeStamp = 0;
        EvenDist_pf06cap(iPv+1).Critical.deltaV.BusNumber = 0;
    else
        % find max deltaV
        lastU = EvenDist_pf06cap(iPv).Results.U_hist;
        diffU = abs(voltageVec) - abs(lastU);
        [rowMaxDiff, timeMaxDiff] = find(diffU == max(max(diffU)));
        iMaxDiff = loadNumber(rowMaxDiff);
        
        EvenDist_pf06cap(iPv+1).Critical.deltaV.Voltage = abs(diffU(rowMaxDiff, timeMaxDiff));
        EvenDist_pf06cap(iPv+1).Critical.deltaV.TimeStamp = timeMaxDiff;
        EvenDist_pf06cap(iPv+1).Critical.deltaV.BusNumber = iMaxDiff;
    end
